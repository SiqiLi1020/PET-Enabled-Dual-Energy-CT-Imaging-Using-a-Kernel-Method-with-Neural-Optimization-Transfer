function [u, x, out] = psct_kmlaa_OT(yi, ni, A, Aopt, u, G, Gopt, x, ri, maxit, K_CT, sub_iteration)
%
...
% Modified by Siqi Li
% Add neural network learning part in attenuation reconstruction at line 140: funtion "attn_ml_sps_OT"
...
% sub-iteration for neural network learning
if nargin<12 | isempty(sub_iteration)
    sub_iteration = 151;
end
...
for it = 1:maxit     
    ...
    if Aopt.attnFlag 
        if it == 1
            sub_iter = 0;
            save('sub_iter.mat','sub_iter');
        else 
            sub_iter = sub_iteration;
            save('sub_iter.mat','sub_iter');
        end
        mi = ni.*li_emis;
        [u, V] = attn_ml_sps_OT(yi, mi, A, Aopt, u, ri, it_attn, li_attn, aa);
        li_attn = proj_forw(A, Aopt, u);
